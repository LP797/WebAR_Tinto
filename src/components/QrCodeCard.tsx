...
export default function QrCodeCard({ codigo }: Readonly<Props>) {
  /* Use window.location.origin at runtime so QRs match the actual host
     (ngrok, Cloudflare Pages, etc.) without rebuilding. Falls back to
     BASE_URL during SSR / before hydration. */
  ...
  const url = `${baseUrl}/maquina/${codigo}`
